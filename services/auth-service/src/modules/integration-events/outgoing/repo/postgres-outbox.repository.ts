...
export const createPostgresOutboxRepository = (sql: DatabaseClient): OutboxRepository => ({
  append: async (event, aggregateId) => {
    await sql`
  ...
  claimDue: async ({ limit, leaseMs, leaseOwner }) => {
  ...
  markPublished: async ({ eventId, leaseOwner }) => {
  ...
  markFailed: async ({ eventId, leaseOwner, error, nextAttemptAt }) => {
    const [row] = await sql<{ id: string }[]>`
      UPDATE outbox_events
      SET next_attempt_at = ${nextAttemptAt}, locked_until = NULL,
          lease_owner = NULL, last_error = ${error}
      WHERE id = ${eventId}
        AND lease_owner = ${leaseOwner}
        AND locked_until > now()
        AND published_at IS NULL
  ...
  markDeadLettered: async ({ eventId, leaseOwner, error }) => {
    const [row] = await sql<{ id: string }[]>`
      UPDATE outbox_events
  ...
  replayDeadLettered: async eventId => {
    const [row] = await sql<{ id: string }[]>`
      UPDATE outbox_events
