...
/** SQL-adapter for the user_profiles table. */
export const createUserProfileRepository = (sql: DatabaseClient) => ({
  createIfAbsent: async (userId: string): Promise<void> => {
    await sql`
  ...
  update: async (
    userId: string,
    input: Partial<Pick<UserProfile, 'displayName' | 'avatarUrl' | 'locale' | 'timezone'>>,
