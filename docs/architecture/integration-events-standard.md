...
Примеры именования ниже отражают целевое соглашение; существующий код ещё
требует переименований, перечисленных в этом стандарте.

## Гарантии доставки

`auth-service` сохраняет account и исходящее событие в одной транзакции. Worker
...
Worker резервирует только доступный batch и обрабатывает его параллельно. Claim
...
5. Экспортировать сборку обработчиков из business module отдельно от его
   HTTP-плагина и передать реестр в integration-events. Полный тип
   `IntegrationEventHandlers` заставляет добавить ветку для каждой принятой
   версии.
